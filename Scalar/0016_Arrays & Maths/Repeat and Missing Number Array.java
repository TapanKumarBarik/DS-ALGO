 ...
 * Food for thought :
 * 
 * Even though it might not be required in this problem, in some cases, you
 * might be required to order the operations cleverly so that the numbers do not
 * overflow.
 * For example, if you need to calculate n! / k! where n! is factorial(n), one
 * approach is to calculate factorial(n), factorial(k) and then divide them.
 * Another approach is to only multiple numbers from k + 1 ... n to calculate
 * the result.
